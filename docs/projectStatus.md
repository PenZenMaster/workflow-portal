...
1. Issue #30 / Epic 3 (Measurement Health) is FULLY COMPLETE as of v1.75.0 (closed on GitHub 2026-08-10) - no further work scoped under it. Next candidates: pick an epic (Epic 1 Platform Integration Assurance, or Epic 7 Client Executive Report) or continue tech debt - only TD-23 (recommendation overrides don't survive a re-parse) remains open at Medium severity; TD-12/TD-13 are Low. Also still open, not yet actioned: TD-22's data note - 67 production runs across nearly every client still carry citations collapsed to "co.uk"/"com.au" (98 + 94 = 192 citations, grown from the original 35 found 2026-07-15 - see part 9 below for the exact run list) need a per-run re-parse to pick up corrected root domains. Not yet run as of this checkpoint - user's call on timing/which runs, or ask to have it scripted as a bulk operation.
...
citation volume has grown since then). Walked the user through the
mechanism: per-run "Re-parse responses" button on Run Detail
(POST /api/runs/:id/reparse, EDITOR_ROLES) re-enqueues parse-response
for every completed response in that run - reads live from
responses_raw, no separate migration/backfill needed. Not run yet as
of this checkpoint - user's call on timing/which runs, or whether to
have it scripted as a bulk operation instead of 67 manual clicks.
